    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 psMain (Varyings input) : SV_TARGET {
                ...
                float colorh = length(color * kernel1);
                float colorv = length(color * kernel2);
                float mag = length(float2(colorh, colorv));
                float4 colormag = float4(mag, mag, mag, 1.0);
                //return kernel;
                return lerp(color, colormag, _factor);
            }
            ENDHLSL
        }
    }
